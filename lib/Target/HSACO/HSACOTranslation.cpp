...
std::string generate_hsaco(llvm::Module *module, const std::string &triple,
  ...
  std::string lld_suffix = "/llvm/bin/ld.lld";
  std::string lld_path;
  if (rocm_path.empty()){
    std::cout << "ROCM_PATH Not found...Trying /opt/rocm" << std::endl;
    rocm_path = "/opt/rocm";
  } 

  lld_path = "/opt/rocm" + lld_suffix;
  if (!std::filesystem::exists(lld_path)) {
    std::cout << lld_path << " Does not exist...Checking Locally" << std::endl;
    ...
    lld_path = compiletime_path.string();
  }
